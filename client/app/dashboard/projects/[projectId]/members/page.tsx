...
export default function ProjectMembersPage() {
  ...
  if (!project) return <div className="animate-pulse h-40 bg-gray-100 rounded-xl" />;
    ...
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">{project.members.length} member{project.members.length !== 1 ? 's' : ''} in {project.name}</p>
        </div>
        ...
        {project.members.map(m => (
          <div key={m.user._id} className="flex items-center gap-4 p-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-medium text-gray-900">{m.user.displayName || m.user.username}</p>
