...
export default function ProjectPage() {
  ...
  if (loading) {
    return <div className="animate-pulse space-y-4"><div className="h-8 bg-gray-100 rounded w-48" /><div className="h-32 bg-gray-100 rounded" /></div>;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-xl shrink-0" style={{ background: project.color }} />
          <div>
        ...
        <div className="flex gap-2">
          <Link href={`/projects/${projectId}/members`} className="btn-secondary">
            <Users className="w-4 h-4" /> Members
          </Link>
          <Link href={`/projects/${projectId}/settings`} className="btn-ghost">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {boards.map(board => (
            <Link key={board._id}
                  href={`/projects/${projectId}/board/${board._id}`}
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-gray-500">Click to open board</p>
              </div>
            </Link>
          ))}
          {boards.length === 0 && (
            <div className="card p-8 text-center col-span-full">
              <p className="text-gray-500 text-sm">No boards yet. Click &quot;Add Board&quot; to create one.</p>
            </div>
          )}
        </div>
      </div>
            ...
            {project.members.slice(0, 5).map(m => (
              <div key={m.user._id} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-900 truncate">{m.user.displayName || m.user.username}</p>
                </div>
            ...
            <div className="space-y-3">
              {activity.slice(0, 8).map(log => (
                <div key={log._id} className="flex items-start gap-3 text-sm">
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="font-medium text-gray-800">{log.user?.displayName}</span>
                    {' '}<span className="text-gray-500">{log.action.replace('.', ' ')}</span>
                    {log.task && <span className="text-gray-700"> &ldquo;{log.task.title}&rdquo;</span>}
